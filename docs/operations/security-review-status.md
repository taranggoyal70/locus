...
| --- | --- | --- |
| R19 | P2 *monitor on first open* | Substantially mitigated, not yet exercised under load. Two signup-side controls now exist alongside the per-account limits: self-serve requires a verified email address, and `LOCUS_SELF_SERVE_MAX_ACCOUNTS` caps how many accounts self-serve will admit at all. Together they bound total exposure to a number the operator sets rather than to how fast someone can script Clerk signups, which is what the earlier per-account-only posture left open. Both apply only to an account with no Admission record, so neither can evict an admitted account or lock out a partner or subscriber. Residual: a verified address is a tedium barrier, not an identity one — disposable inboxes defeat it — so an attacker willing to spend time can still consume the whole ceiling, and the ceiling is then the only thing standing. Accepted for a first open with a deliberately small ceiling and the `admission_resolved` analytics watched for a spike in `at_capacity`. Revisit before raising the ceiling materially or before free-tier Runs get a larger token budget; payment details for anything above `free` remain the durable answer.
