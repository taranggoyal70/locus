...
describe("authenticated product routes", () => {
  it("protects every product entry while leaving authentication pages public", () => {
    for (const path of ["/workspace", "/workspace/settings", "/settings", "/projects", "/api/github", "/api/agent/runs", "/api/attachments", "/api/keys", "/api/projects", "/api/usage", "/repos/taxonomy.json"]) {
      expect(isProtectedPathname(path), path).toBe(true);
    }
    for (const path of ["/", "/demo", "/sign-in", "/sign-up", "/icon.svg", "/api/v1/locate", "/api/health", "/docs"]) {
      expect(isProtectedPathname(path), path).toBe(false);
    }
  });

  it("routes signed-out browser pages through the branded account flow", () => {
    for (const path of ["/workspace", "/workspace/settings", "/settings", "/projects"]) {
