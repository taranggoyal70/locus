...
export function classifyAgentFailure(error: unknown): AgentFailureKind {
  ...
  if (
    /provider|language model|ai_apicallerror|model response|codingagenttimeouterror|coding agent execution exceeded its deadline|ai_nooutputgeneratederror|no output generated/.test(
      text,
    )
  ) {
